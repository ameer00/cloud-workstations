...
| ID | Feature | Spec | Priority | Status | Owner | Branch | Dependencies | Feedback |
|----|---------|------|----------|--------|-------|--------|--------------|----------|
| F-0108 | Remap workspace assignments (ws1-3 foot, ws4 Chrome, remove Antigravity) | [F-0108](specs/F-0108-workspace-autostart-remap.md) | P0 | implemented | SWE-1 | feature/workspace-remap | — | Modify 08-workspaces.sh: ws1/2/3=foot, ws4=Chrome. Remove Antigravity autostart block entirely. Update header comments and log messages. |
| F-0109 | Root-cause and fix Chrome autostart (window never appears) | [F-0108](specs/F-0108-workspace-autostart-remap.md) | P0 | implemented | SWE-1 | feature/workspace-remap | F-0108 | Root cause: google-chrome-stable only exists at ~/.nix-profile/bin/ which is NOT on the minimal system PATH used by runuser. Fix: launch Chrome via swaymsg exec (inherits full sway session env + Nix PATH), drop --ozone-platform=wayland (Nix wrapper handles it). |
| F-0110 | Add boot tests for new workspace layout | [F-0108](specs/F-0108-workspace-autostart-remap.md) | P0 | tested | SWE-Test | feature/workspace-remap | F-0108, F-0109 | 6 tests verified by SWE-Test: syntax OK, mutation check passes (all 6 discriminate old vs new), no stale tests, setup-script deploy confirmed. Ready for PE deployment. |
